...
const CreateChild = () => {
  ...
  useEffect(() => {
    if (planName !== "Free") {
      ...
      const createdProfiles = parseInt(
        localStorage.getItem(`profilesCreated_${userId}_${planName}`) || "0",
        10
      );
  ...
  const submit = async () => {
    ...
    const currentCreatedProfiles = parseInt(
      localStorage.getItem(`profilesCreated_${userId}_${planName}`) || "0",
      10
    );
    ...
    try {
      ...
      if (response.ok) {
        ...
        localStorage.setItem(
          `profilesCreated_${userId}_${planName}`,
          updatedProfileCount
        );
        ...
        const currentPlanCount = parseInt(
          localStorage.getItem(`${planName.toLowerCase()}UserCount`) || "0",
          10
        );
        localStorage.setItem(
          `${planName.toLowerCase()}UserCount`,
          currentPlanCount + 1
        );
  ...
  return (
    <div className="parentHome">
      ...
      <div className="parent-welcome-header">
        ...
        <p>
          You have created {createdProfiles} out of {maxProfiles} profiles under
          the {planName} plan.
        </p>
        <button
        ...
        >
          ...
          {hasReachedLimit && (
            <p className="error-msg">
              You have reached the maximum number of profiles for the {planName}{" "}
              plan.
            </p>
          )}
        </form>
